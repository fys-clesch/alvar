                ...
                const uint len_fname, uchar *__restrict time_data)
{
    ...
    if(argc != 3 && argc != 4)
    {
        fprintf(stdout,
                ...
                "           .1 -1.2984 .4 2.498 ...\n" \
                "           ...\n" \
                "           The maximum linelength is 511 characters.\n" \
                "           Allowed whitespaces are ',' ';' '\\t' ' '.\n" \
...
/** \brief Converts a file containing floating point numbers into a double matrix.
 *
 * \param fname const char*__restrict The filename.
 * \param out double *__restrict The output matrix.
 * \param row const uint The number of rows of the output matrix, must be known first.
 * \param col const uint The number of columns of the output matrix, must be known as well.
 * \param chatty const uchar Spits out some more information.
 * \return uint Returns the number of entries. Returns 0 in case of an error.
 *
 */
uint fpfile2double(const char *__restrict fname, double *__restrict out, const uint row, const uint col, const uchar chatty)
{
    ...
    while(fscanf(readfile, " %511[^\n]", templongstr) != EOF) /**< The actual break is not read, it will be consumed in the next turn */
    {
        ...
        if(s > row * col)
        {
            ERR_INTRO "we are running over the allocated memory!\n", ERR_ARG);
            fclose(readfile);
            return 0;
        }
        memset(&templongstr[0], 0, sizeof(templongstr));
    }

    if(ferror(readfile))
        ERR_INTRO "file error '%s'\n", ERR_ARG, fname);
    fclose(readfile);
    ...
    if(chatty)
        fprintf(stdout,
                ...
                "  has %u entries\n"
                "  has %u comment lines\n"
                "  forms a %u x %u matrix (row x 0)\n",
                fname, s, ncom, i, j);

    assert(i == row && s == row * col); /**< Sanity check */
                ...
                uint *__restrict cmt_lines)
{
    ...
    *nrow = sum / (*ncol);
    fprintf(stdout,
            ...
            "  has %u entries\n"
            "  has %u comment lines\n"
            "  forms a %u x %u matrix (row x col)\n",
